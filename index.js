...
function getFinals(data) {
    let final = fifaData.filter(data => data.Stage === 'Final');
    return final;
}
...
function getYears(array, func) {
    let finalsYears = func(array).map(data => data.Year);
    return finalsYears;
}
...
function getWinners(array, func) {
    let finalTeams = func(array);
    let winners = finalTeams.map(data => {
...
function getWinnersByYear(array, winningYear, winningCountry) {
    ...
    for (let i = 0; i < country.length; i++) {
        newArray.push('In ' + year[i] + ', ' + country[i] +  ' won the world cup!');
    }
...
//    console.log(averageGoals / func.length).toFixed(2);
