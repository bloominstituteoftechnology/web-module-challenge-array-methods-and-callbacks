...
function getWinners(array,callback) {
    ...
    getData.forEach(element => {
        if(element["Home Team Goals"] > element["Away Team Goals"]){
            ...
            // console.log("GET DATA",getData["Home Team Name"]);
        }else{
...
function getWinnersByYear(array, getYearsCB, getWinnersCB ) {
    ...
    year.forEach(function(item,index){
        winnerByYear.push(`In ${item}, ${country[index]} won the world cup!`)
...
function getAverageGoals(getFinalsCB) {
   ...
   let home = getFinalsCB(fifaData).reduce(function(acc,item){
       return acc + item["Home Team Goals"];
   },0);
   let away = getFinalsCB(fifaData).reduce(function(acc,item){
    return acc + item["Away Team Goals"];
},0);
    home = home / 19;
    away = away / 19;
    
   let message = `Home: ${home.toFixed(2)} Away: ${away.toFixed(2)}`;
   return message;
}
