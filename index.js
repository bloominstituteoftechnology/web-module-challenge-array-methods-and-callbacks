...
function getFinals(data) {
    const onlyFinalsData = fifaData.filter(function(array){
...
function getYears(callback){
    let years = callback.map(callback => callback["Years"]);
    return years;
}
...
function getWinnersByYear(cbWinners, cbYears) {
    let winByYear = cbYears.map(function(item, index){
...
function getAverageGoals(data) {
    let homeAverage = data.reduce((accumulator, item) => accumulator + item["Home Tome Goals"], 0) / data.length;
