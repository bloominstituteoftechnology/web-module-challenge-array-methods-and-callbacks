...
let fourteen = fifaData.filter((year) =>{
...
function getYears(data, cb) {
    return cb(data).map((team) => team.Year)
}
...
function getWinners(data, cb) {
    return cb(data).map((game) =>  game["Home Team Goals"] > game["Away Team Goals"] ? game["Home Team Name"] : game["Away Team Name"])
}
...
function getAverageGoals(data) {
   let avarage = data.reduce((prev, curr) =>{

        return (curr["Home Team Goals"] + curr["Away Team Goals"]) + prev
   },0)/ data.length
   return avarage.toPrecision(3)
}
