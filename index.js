...
let final = fifaData.filter((item)=> {
...
function getAverageGoals(arr) {
   const homeGoals = []
   arr.map((item)=>{
       homeGoals.push(item["Home Team Goals"])
   })


   const awayGoals = []
   arr.map((item)=>{
    awayGoals.push(item["Away Team Goals"])
})

    const total = homeGoals.reduce((a,b)=> a+b, 0) + awayGoals.reduce((a,b)=> a+b, 0);
